...
class BlueprintScene: BaseScene {
    ...
    private func showHub() {
        ...
        createTaskButton(
            emoji: "👕", label: "Clothes", done: clothesDone, y: size.height * 0.65,
            name: "task_clothes", topic: "Inheritance")
        createTaskButton(
            emoji: "🍱", label: "Lunch", done: lunchDone, y: size.height * 0.45, name: "task_lunch",
            topic: "Encapsulation")
        createTaskButton(
            emoji: "🎒", label: "Bag", done: bagDone, y: size.height * 0.25, name: "task_bag",
            topic: "Polymorphism")
        ...
        if clothesDone && lunchDone && bagDone {
            dialogBox.showDialog(
                name: "Robot",
                text:
                    "🎉 Tori is COMPLETE! All properties are set. You learned OOP by getting ready for school!"
            )
        } else {
            let remaining = 3 - [clothesDone, lunchDone, bagDone].filter { $0 }.count
            dialogBox.showDialog(
                name: "Robot",
                text: "Tori needs \(remaining) more things! Tap a task to learn OOP concepts.")
        }
    }

    private func createTaskButton(
        emoji: String, label: String, done: Bool, y: CGFloat, name: String, topic: String
    ) {
    ...
    private func showClothesGame() {
        ...
        dialogBox.showDialog(
            name: "Robot",
            text:
                "INHERITANCE: Shirt inherits from Clothing! It gets color property automatically, plus adds its own (hasButtons). Tap Done when ready!"
        )
    }
    ...
    private func showLunchGame() {
        ...
        dialogBox.showDialog(
            name: "Robot",
            text:
                "ENCAPSULATION: The lunchbox HIDES your food (private). You can only access it through methods like eat()! Data is protected."
        )
    }
    ...
    private func showBagGame() {
        ...
        dialogBox.showDialog(
            name: "Robot",
            text:
                "POLYMORPHISM: Pen and Pencil both have write(), but they work differently! Same interface, different implementation."
        )
    }
    ...
    override func handleTouch(at location: CGPoint, touch: UITouch) {
        ...
        for node in tappedNodes {
            ...
            if name == "task_clothes" && !clothesDone {
                showClothesGame()
                return
            }
            if name == "task_lunch" && !lunchDone {
                showLunchGame()
                return
            }
            if name == "task_bag" && !bagDone {
                showBagGame()
                return
            }
